...
**A note on what a phase costs.** From the standard band up, review dispatches each substantive lens to a fresh-context subagent, and its risky band fans out further still, per lens and per pull request. That is deliberate, since a lens that recomputes finds what a lens that recalls does not, but it means a single `/deliberate-engineering:review` on a risky change can run many agents rather than one. The ceremony band is the lever: classify honestly and a trivial change stays a single light pass.
