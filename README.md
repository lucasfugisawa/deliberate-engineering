...
- **`deliberate-engineering-rules`**: nine standing rules held across every phase: the human keeps the trigger on irreversible and outward-facing actions, claims are checked against primary evidence before they're endorsed, recommendations arrive as a reasoned pick rather than a bare menu, durable state is checkpointed before compacting, and nothing ships the reader can't resolve. Scoped to software work; quiet on research, prose, and ad-hoc analysis. To override one you need its number, and the numbered list lives in the skill itself: [the standing rules themselves](plugins/deliberate-engineering/skills/deliberate-engineering-rules/SKILL.md).
...
**A note on what a phase costs.** From the standard band up, review dispatches each substantive lens to a fresh-context subagent, and its risky band fans out further still, per lens and per unit of review. That is deliberate, since a lens that recomputes finds what a lens that recalls does not, but it means a single `/deliberate-engineering:review` on a risky change can run many agents rather than one. The ceremony band is the lever: classify honestly and a trivial change stays a single light pass.
...
<details>
<summary><strong>The four phase catalogs in detail</strong> (117 strategies total; the cross-cutting communication catalog adds seven lenses)</summary>
...
- **Verification: 25 strategies** in five groups: evidence & ground truth (7), local & pre-merge (6), staged promotion & rollout (5), post-deploy production verification (5), and operational data-mutation verification (2). For establishing something is *actually* true, with evidence from running systems, not just plausible on paper. Review asks "does this look correct?"; verification asks "is it correct, and what's my evidence?"
...
The profile is a directory of small markdown files, and one of them, `chat-prompt.md`, is a self-contained rendering you can paste into any other tool's custom-instructions field. Register files are named by ISO 639-1 code, so `en.md` and `de.md`. When no register matches the language you are writing in, the agent says so and applies the rest of the profile. There are two ways to get a profile: write the files yourself against the [contract](plugins/deliberate-engineering/skills/deliberate-engineering-voice/contract.md), which ships a worked `core.md` for an invented persona to calibrate density and citation style against, or build one properly. The skeleton takes minutes; the rules in it are the work. To build one, follow the **[voice-build guide](docs/guides/voice-build.md)**: `/deliberate-engineering:voice-build` is a guided, resumable flow that takes you from identifying your communication archetypes through collection, analysis, and synthesis to a finished profile, keeping the two steps that need your judgment (the style interview and the blind A/B) in your hands. The full format lives in the skill's [contract](plugins/deliberate-engineering/skills/deliberate-engineering-voice/contract.md); the underlying method in its [bootstrap guide](plugins/deliberate-engineering/skills/deliberate-engineering-voice/bootstrap.md).
