...
This is your machine's choice, never a requirement of the plugin: it only changes *when* the router and rules fire on your machine. One consequence worth knowing before you skip it: a standing rule you wrote in your override file is read by the rules skill, so a session where that skill does not fire is a session where your rule goes unread. If you keep standing rules, the always-on block is what makes them reliable rather than occasional. To undo it, see [Uninstall](#uninstall).
...
- **`deliberate-engineering-rules`**: nine standing rules held across every phase: the human keeps the trigger on irreversible and outward-facing actions, claims are checked against primary evidence before they're endorsed, recommendations arrive as a reasoned pick rather than a bare menu, durable state is checkpointed before compacting, and nothing ships the reader can't resolve. Scoped to software work; quiet on research, prose, and ad-hoc analysis. To override one you need its number, and the numbered list lives in the skill itself: [the nine standing rules](plugins/deliberate-engineering/skills/deliberate-engineering-rules/SKILL.md).
...
**A note on what a phase costs.** From the standard band up, review and verification dispatch each substantive lens to a fresh-context subagent, and the risky band fans out further still. That is deliberate, since a lens that recomputes finds what a lens that recalls does not, but it means a single `/deliberate-engineering:review` on a risky change can run many agents rather than one. The ceremony band is the lever: classify honestly and a trivial change stays a single light pass.
...
- **Make it yours**: a personal override layer lets your own practice take precedence over any shipped lens, composition pattern or rule; `/deliberate-engineering:capture` distills a session into ready-to-paste override blocks. See [Make it yours](#make-it-yours).
...
The plugin is opinionated, and it's meant to become yours. A personal override file takes precedence over any shipped lens, composition pattern or rule it can address, which is every catalog lens, every numbered composition pattern and every standing rule, but not the router's own classification axes or its genre-to-phase map, which stay architecture. Entries are addressed by stable id (`review #35`, `verify #14`, `review pattern #7`, `Rule 2`): disable one you don't want, annotate one with a note of your own, or add your own: a strategy, a composition pattern, or a standing rule. The agent always declares when an override changed what it did, and you can even loosen a safety rule, which it honors while calling out the raised autonomy. It's opt-in: no file, no change.
...
The profile is a directory of small markdown files, and one of them, `chat-prompt.md`, is a self-contained rendering you can paste into any other tool's custom-instructions field. Register files are named by ISO 639-1 code, so `en.md` and `de.md`, and a file named `english.md` silently never loads. There are two ways to get a profile: write the files yourself against the [contract](plugins/deliberate-engineering/skills/deliberate-engineering-voice/contract.md), which takes minutes and includes a worked example profile to calibrate against, or build one properly. To build one, follow the **[voice-build guide](docs/guides/voice-build.md)**: `/deliberate-engineering:voice-build` is a guided, resumable flow that takes you from identifying your communication archetypes through collection, analysis, and synthesis to a finished profile, keeping the two steps that need your judgment (the style interview and the blind A/B) in your hands. The full format lives in the skill's [contract](plugins/deliberate-engineering/skills/deliberate-engineering-voice/contract.md); the underlying method in its [bootstrap guide](plugins/deliberate-engineering/skills/deliberate-engineering-voice/bootstrap.md).
...
It also commits. A commit on a local branch is treated as ordinary work rather than a gated action, on the reasoning that the gate exists for what leaves your machine or cannot be undone, and a local commit is neither. Push, merge, tag and release all stop for you. If you would rather it did not commit at all, say so in the session or write it as a standing rule.
...
**What stays on disk, on purpose.** Removing the plugin removes the product, not the things it wrote for you, because those are yours and a reinstall should find them. If you want them gone too: `~/.claude/deliberate-engineering/` holds your override file, your voice profile, your voice-build working directory and any state notes taken outside a repository, and `.deliberate/state/` holds the per-branch notes inside each repository you worked in. The voice-build working directory is the one worth a deliberate look, since it holds the writing corpus you assembled.
