...
Run the cross-session loop: plan the program (via the router and selectors), decide per unit whether to do it inline or dispatch it, and if dispatched whether to a fresh human-watched session (the default) or a background subagent (the narrow earned band). Author each handoff as a self-contained contract with its premise frozen against a live-state read, verify every return at source (in a separate context for contested or high-stakes claims), disposition it accept/reject/follow-up, and keep the program tracker and recovery anchor current on every checkpoint. Stop at every irreversible or outward-facing action for the operator to trigger. If the work fits one session, say so and route it through the phases directly instead.
