...
description: "Use when work spans phases or sessions and process state must survive: the current phase and phase sequence, the chosen ceremony band and lenses, open pendings, and the decisions made and why. Reads and rehydrates a per-work-unit working-note at the start of a session or phase, and rewrites it at Rule 6 checkpoints with a changed-since-read safety check. Writes that note into the repository at .deliberate/state/ once it confirms the path is VCS-ignored, and falls back to the current host's Deliberate Engineering data root otherwise. Delegates to a tracker or superpowers when one is present; falls back to its own note otherwise. Consulted by the router and Rule 6, not invoked directly; stays silent when there is no state to record."
---

# Deliberate Engineering State

Consult `deliberate-engineering-host-context` before resolving the fallback. Project-local state remains host-independent; the returned `data_root` supplies only the fallback `<data_root>/state/`.
...
Rule 6 sets the checkpoint posture: compact judiciously, and checkpoint before you do. The router writes the note as a step of each phase transition when the work spans phases or sessions. But neither defines where to write the note or what schema it follows: they own when the write fires, not the mechanism. This skill IS that mechanism. It does not replace Rule 6's posture; it gives it a concrete home. The founding "decide and delegate, don't invent a file format" principle is preserved: the router and Rule 6 still don't invent format inline. They delegate here, and THIS skill is the one place the format and location logic live.
...
2. **Fallback when project-local is unavailable**: `<data_root>/state/`, resolved for the current host. Use this when there is no repository root or the project-local ignore setup failed.
...
One note per work-unit, named by a world-derived identifier in preference order: the current branch name, the PR number, or the ticket key, falling back to an assigned name when the work-unit has none of the three (below). The filename is `<identifier>.md`, where path-unsafe characters in the identifier (e.g., `/` in a branch name like `feat/user-export`) are sanitized (e.g., to `-`) to produce a valid filename like `feat-user-export.md`. Other examples: `pr-1234.md`, `PROJ-42.md`. By construction, parallel sessions working on different work-units never collide: each has its own note. One exception, and it is the branch case: when the branch is a long-lived shared one (`main`, a release branch, a standing integration branch), the branch name identifies the branch and not the work unit, so it is not a usable identifier. Fall through to the PR number or the ticket key, and to an assigned name when neither exists, under the same rule that two different work-units must never be given the same name. Say which identifier you used and why the branch was skipped. The rare collision case is two sessions working on the same work-unit simultaneously; the write gate (below) handles that with best-effort detect-and-reconcile.
