...
Strategy **numbers are stable identifiers, not reading order**: a lens keeps its number for life, and a new lens is appended with the next free number and *placed* under the group it belongs to. So a group may run out of numeric sequence: Part A runs 1 to 4, then 54, then 5 to 11, then 52 and 53. This is deliberate: it keeps every published number citable (e.g. by an override) without renumbering.
...
- **How it works:** A review deliberately *without* the history of prior edits, so as not to inherit rationalizations already made (e.g. dispatch a reviewer with no context of who edited). Give it two jobs, not one: find what the earlier passes missed, and say what it would defend the artifact against if someone flagged it as a defect. A reviewer only asked to find things finds things.
- **Objective:** Break the "each pass finds less because it is already convinced" loop, and catch a finding the earlier passes invented rather than found.
...
- **5. Discovery before remediation:** Keep a review pass read-only: report what you find and stop, rather than fixing as you go. Read-only is about the artifact under review, not a ban on running anything: that is why lens 12 executes in a disposable copy, since running code in place leaves build artifacts behind and those are a change too. Findings stay trustworthy and complete when the audit doesn't mutate the artifact mid-pass; remediation is a separate, later step.
...
- **8. Do not hand a dispatched agent your hypothesis:** When you dispatch a lens, describe what to examine, never what you expect to be wrong. A prompt that says "find where this truncates" will be answered even when nothing truncates, and the answer comes back with your own confidence attached. This is the failure mode of a design that dispatches its lenses: contamination travels in the brief, so no number of independent agents dilutes it. Say what to look at, what evidence would settle it, and that reporting nothing found is a result.
