...
Deliberately skipped, and logged as Step 4 requires: **17** (no parsing, aggregation, time-window or pagination surface, though it shares a bullet with the selected 12), **47** (the backfill rewrites existing rows and evolves no schema, which is the one Part-E key, *data*, that this change could have matched), **51** (no boundary moves, no new component), **14** (single checkout), **19** (nothing contradicts a norm), **33** (no new type), **36** (no dependency change), **45** and **46** (no resource provisioning, no retention change, not a critical-path service), and the rest of the Part-E domain group (no frontend, mobile, infra or experiment surface).
