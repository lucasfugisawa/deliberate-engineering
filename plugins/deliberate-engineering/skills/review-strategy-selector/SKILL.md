...
- **Risky / irreversible / ambiguous / money-or-data** → full adversarial depth in fresh-context fan-out. The unit is whatever the review is scoped to, a pull request where there is one and the artifact itself where there is not, split per lens for the critical ones (25 and 28 always; 26 where the change has an attack surface, which touching money does not by itself imply). Group the rest three or four to an agent rather than one each: a dozen lenses on a small artifact is a dozen re-reads of the same file, and independence stops buying anything once it costs more than the artifact is worth. adversarial majority-refute (2, 9) on top; where the design space is wide and being wrong is expensive, run 8 judge panel (several independent attempts, scored, the best synthesized); close with a fresh-eyes pass (3) run as a *literally separate* fresh-context agent, not an in-context re-read.
...
- **Before anything else, if the artifact may be partial** → establish whether it is whole enough to judge: is there an entrypoint, do the callers exist, is the history contiguous? Otherwise findings blur between built wrong and not built yet, and you will weight them wrongly.
