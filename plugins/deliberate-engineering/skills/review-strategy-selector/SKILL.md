...
**Worked example (change writes production data via a backfill):** Non-trivial on Risk + Reversibility + Reach. Selected lenses: **25** (functional correctness: does it backfill what the requirement meant?), **11** (change-size: is the backfill reviewable, or does it ride along with unrelated work?), **34** and **35** (the source it adds, and whether any of it is unneeded), **23** (concurrency: backfill contends with live writes), **24** (reversibility: is there a bounded way back?), **28** (data integrity: invariants/transactions hold?), **42** (privacy: the rows it rewrites carry personal data), **55** and **29** (Reach: every consumer of the rewritten rows, and the contracts they read them through), **2** (adversarial: try to break it), closed with **3** (fresh eyes). Deliberately skipped and logged: the Part-E domain group (no frontend, mobile, infra or experiment surface), **12** (the semantics here are not subtle enough to need execution), and **51** (no boundary moves).
