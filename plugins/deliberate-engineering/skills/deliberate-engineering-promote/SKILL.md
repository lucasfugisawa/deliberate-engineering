...
description: "Use on demand to drive a pending candidate into the shared catalog through a leak audit, isolated-versus-structural classification, append-only editing, selector routing, fresh-context review, validation, and a human publication gate. The isolated route edits only the plugin repository working tree and removes the promoted candidate after review; the structural route stops and recommends a full design cycle. It never commits, opens a PR, or pushes, and stays silent unless invoked."
---
...
**`operation: modify` with `modifies: N`, `kind: pattern`**. The candidate amends an existing composition pattern. Locate the `- **N. Title:**` bullet in the catalog's appendix, where N matches `modifies` in the pattern namespace, and edit it in place: same number, same appendix, same position. If the amendment changes the pattern's name, change it in the bullet and in every selector compose step that cites it, since the guard matches a citation by the pattern's own name. If that bullet does not exist, stop: this is the modify-nonexistent error below, in the pattern namespace.
...
Appending a lens changes numbers that several files state, and `scripts/check-consistency.sh` enforces them. Before handing the working tree over, from the repo root run `bash scripts/check-consistency.sh` with a bash 4 or newer (macOS ships bash 3.2 as `/bin/bash`, which cannot run it; use the one from your package manager) and fix what it names. Then run `python3 scripts/check-invariants.py --base origin/main` and `python3 scripts/check-host-compatibility.py` from the same root: they prove the append-only numbering and reachability invariants held and that both host adapters still point at the same shared methodology. The obligations they enforce, so you can update them in the same edit rather than discovering them from a red run: the catalog's own intro count ("this catalog contains N"), the README's per-catalog count and its per-group parentheticals, the README's total across the four phase catalogs, and the communication count where the README and the architecture doc both state it. A change under `plugins/deliberate-engineering/` also requires the Claude plugin manifest, Codex plugin manifest, and version-bearing Claude marketplace entry to move in lockstep; the Codex marketplace schema carries no version. A CHANGELOG entry is required by convention and is not machine-checked, so nothing will stop you from forgetting it.

### Run a fresh-context skill review

After editing the catalog file in the working tree, dispatch a fresh-context subagent through the current host's native subagent mechanism. A specialized skill reviewer may be used when installed, but is not required. Pass the catalog file path resolved above, the selector file you routed it from, and the context that a new lens was added or an existing lens was modified. Instruct the reviewer to check consistency with existing lenses, formatting adherence, clarity of the principle, whether the lens fits the catalog's voice, and whether its selector routing matches its trigger. Require an explicit pass or actionable findings with file-and-line evidence.
...
**Fresh-context review rejection:** report the reviewer's findings, do NOT remove the candidate file, do NOT commit over the rejection. The catalog edit sits in the working tree (reversible). The author addresses the findings (edit the catalog in place or revise the candidate) and either re-invokes promotion or commits manually.

**Fresh-context review unavailable (the host cannot dispatch a subagent):** treat it like a rejection: report that the review step could not run, leave the catalog edit in the working tree, and do NOT remove the candidate file. The review is not optional; never silently skip it and remove the candidate. The author reviews the edit by hand in a separate context before committing or re-invokes promotion from a host that supports subagents.
...
**Malformed candidate (missing frontmatter, invalid `target`, unparseable):** report the malformation, do NOT guess, do NOT edit. A valid `target` is one of `review`, `verify`, `planning`, `debug`, `communication`, or `rules`; a valid `kind` is `lens` or `pattern`, and a candidate written before that field existed carries none, which reads as `lens`; a `kind: pattern` candidate with `target: communication` is malformed, because that catalog has a prose composition note and no numbered appendix; anything else is malformed. Ask the author to fix the candidate file and re-invoke promotion.
...
3. **What was edited:** the catalog file path, the specific lens block added/modified, the selector file and the bullet that now routes it (or the exemption stated for it in `scripts/routing-exemptions.txt`), the fresh-context review result (pass or findings).
