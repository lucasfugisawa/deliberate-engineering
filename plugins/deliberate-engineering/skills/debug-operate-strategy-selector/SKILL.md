...
- **Diagnose under uncertainty** (Part B) → **4** read-the-failure-signature (a resource climbing while load stays flat is a feedback loop, not demand, and the signature also bounds the fix: a self-amplifying failure rarely tolerates one large corrective move), **5** recover-original-intent before editing unfamiliar code under stress (Chesterton's fence at incident speed). Both sit *on top of* `superpowers:systematic-debugging`, which owns the elimination method.
- **Respond under pressure** (Part C) → **6** revert-over-roll-forward / don't wait for the author, **7** bias-toward-action on escalation when stakes are elevated, **8** an un-actionable page is reassigned or escalated, never left to sit, **17** contain the blast radius when there's no clean revert (shed/rate-limit load, disable the failing path, fail over: a holding action while you work the real fix).
