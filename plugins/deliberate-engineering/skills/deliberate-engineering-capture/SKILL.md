...
- **Targets** use the canonical form: `review #N`, `verify #N`, `planning #N`, `debug #N`, `communication #N`, `<catalog> pattern #N` for a composition pattern, `Rule N` for specific lenses/rules; or `add: <catalog>` for operator-authored strategies where catalog is `review`, `planning`, `verify`, `debug`, `communication`, or `rules`.
