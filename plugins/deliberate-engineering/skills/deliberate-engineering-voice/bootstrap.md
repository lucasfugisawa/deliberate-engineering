...
- **Dispatch two subagents with asymmetric file access.** One may read the profile directory; the other is told the directory exists and is forbidden from reading any part of it. Same request, same context, same length target, one draft each.
