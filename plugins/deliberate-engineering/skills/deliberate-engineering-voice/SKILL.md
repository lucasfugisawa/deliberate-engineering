...
- **On a commit message, repository convention wins on form.** The operator signs commit messages, so the exclusion test above puts them in scope, but they are also parsed by tooling and governed by repository convention, and no `commit` archetype exists. Where the two disagree, the convention decides the form (subject shape, prefix, casing, length) and the profile applies to body prose only.
