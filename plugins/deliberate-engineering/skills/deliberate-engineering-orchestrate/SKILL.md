...
**Operator overrides.** Before applying them, consult `deliberate-engineering-overrides`: if planning #10 or #11, or any other lens this decomposition applies, has an operator override (disable / modify), honor it, and ask for any operator-authored `add: planning` entry too, which carries no shipped number. Take the operator's standing rules in the same consult, since a cold orchestration has no earlier read to inherit and the handoff field below asks for them. Declare every deviation. Carry what fired into each handoff (it has its own field in `templates.md`), so a worker session inherits the operator's calibration instead of rediscovering it, or missing it entirely.
...
When a unit turns out to be an irreversibility cluster, hand it the baton rather than dispatching it: no handoff is written, no worker session receives it, the operator opens the conduction themselves, and the tracker holds a pointer to the conductor doc until it returns (see `deliberate-engineering-conduct`).

The program tracker is the program's durable store, and it is a delegation target for `deliberate-engineering-state` rather than a second store beside it: that skill's hybrid-with-fallback stance sends it to whatever durable home already owns the work-unit, so where the tracker is that home, it records there rather than opening its own working-note. It carries what the tracker's fields can hold; anything the tracker has no field for is a gap to close in the tracker, not a reason to start a parallel store. Do not run two sources of truth. When a cluster of irreversible steps concentrates inside a program (a merge cascade, a deploy chain, a batch data mutation, a teardown), hand it to `deliberate-engineering-conduct`: the tracker goes quiet and holds a pointer to the conductor in the same commit the conductor is born, and the baton returns to the tracker at the conductor's closure. One live cockpit at a time.
