...
**Command notation.** `/deliberate-engineering:capture` is the Claude Code form. In Codex 0.154, choose `capture (deliberate-engineering)` through `/skills` or type `$deliberate-engineering:capture`.
...
2. **[Agent]** Reads the full retained session history from disk when it can resolve and validate it (not its own memory of the conversation), then mines your messages for three kinds of signal: deviations from shipped lenses or rules, recurring patterns the catalog lacks, and ceremony-calibration preferences. Scope is this session unless you explicitly widen it, in words like "capture across all my sessions on this project". Claude Code then reads direct project transcripts outside `subagents/`; Codex queries its thread store for operator-owned root sessions whose recorded working directory is the requested project root. For Codex, capture follows every active thread pointer, reconstructs only the retained ordinal ranges after a revert, and can read both active or archived, plain or compressed rollouts. Compressed history needs the `zstd` executable. The pass fans out: one subagent per chunk of transcript, each handed a file of your own messages. If identity, storage, lineage, provenance, or a required decoder cannot be validated, it reports the degradation and falls back to the live context rather than returning a silently partial corpus.
...
6. **[Agent]** Appends the approved blocks to `<host data root>/overrides.md` (creating it if absent), strictly append-only, and reports what was written. If nothing rose to a durable preference, it says so and writes nothing.
...
|---|---|---|
| Your override file (`disable` / `modify` / `add` blocks, addressed by stable ids) | Capture, append-only, on your approval | `<host data root>/overrides.md`, local, never in a repo |
