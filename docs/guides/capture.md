...
2. **[Agent]** Reads the full session transcript from disk (not its own memory of the conversation) and mines your messages for three kinds of signal: deviations from shipped lenses or rules, recurring patterns the catalog lacks, and ceremony-calibration preferences. Scope is this session unless you explicitly widen it, which you do by asking for it explicitly, in words like "capture across all my sessions on this project", and which then reads every session transcript across every worktree of that project. The pass fans out: one subagent per chunk of transcript, each handed a file of your own messages. And where it cannot read a transcript at all, whether the session identifier is unset, the file is missing or it is unreadable, it says so and falls back to observing the live context.
