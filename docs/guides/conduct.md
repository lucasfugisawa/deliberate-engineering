...
2. **[Agent]** Judges whether the cluster earns a standalone conductor at all, and says so either way. A small cascade stays where it was.
...
7. **[Agent]** After the step: runs the between-step verification battery against expected values stated in advance (never a bare exit code), reads the post-state to confirm the write landed (core for a data mutation), and updates the station table.
8. Repeat 5–7 down the fixed order. **[You]** can call a **hold** (a planned pause with an expiry) or an **abort** (an emergency stop to a named safe state) at any point.
9. **[Agent]** On interruption, writes the session residue: live state, the exact next action, any half-taken step and its re-run safety, so a fresh session resumes without trusting anyone's memory.
10. **[Agent]** At completion: the closure marker, the closeout obligations, and the baton returned to the program tracker if one handed the cluster off.

## Artifacts

| Artifact | Written by | Lives |
|---|---|---|
| The CONDUCTOR doc (inventory, gated groups, station table, wave queue, residue, closure marker) | Conduction session, updated as the cluster advances | Wherever you keep it: the plugin ships the contract, the filled-in doc is yours (inside an orchestrated program, the tracker holds the pointer) |
...
- The full field-level contract: the skill's `templates.md` under `plugins/deliberate-engineering/skills/deliberate-engineering-conduct/` (with example station sets for a git rollout, a data batch, and a teardown)
