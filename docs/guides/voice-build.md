...
**Command notation.** `/deliberate-engineering:voice-build` is the Claude Code form. In Codex 0.154, choose `voice-build (deliberate-engineering)` through `/skills` or type `$deliberate-engineering:voice-build`.
...
2. **[Agent]** Frames the cost honestly, resolves the active host's data root, creates its `voice-build/` working directory outside any repo with a safety check, and asks which languages the profile must cover.
3. **[You + Agent]** Derive *your* archetype set: the agent teaches what an archetype is and presents the suggested menu from the voice contract (DMs, code reviews, design docs, email, social posts, and the rest); you rename, split, drop, and invent until the set is yours.
...
9. **[Agent]** Installs the finished profile at `<host data root>/voice/`; **[You]** decide what happens to the corpus (delete, or keep locally; it never enters a repo).
...
|---|---|---|
| Collection manifest (the authoritative resume state) | The flow | `<host data root>/voice-build/` |
| Corpus samples (typed, era-tagged) | You | Same working dir; disposal is your call at the end |
| The voice profile (`core.md`, `registers/`, `archetypes/`) | Synthesis, validated by your two gates | `<host data root>/voice/` |

The profile format lives in the voice skill's [contract.md](../../plugins/deliberate-engineering/skills/deliberate-engineering-voice/contract.md); the underlying method in its [bootstrap.md](../../plugins/deliberate-engineering/skills/deliberate-engineering-voice/bootstrap.md). This flow runs that method for you; the documents remain the reference.
