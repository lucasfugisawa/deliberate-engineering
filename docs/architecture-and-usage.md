...
The shipped payload is one shared methodology, with two thin discovery adapters:

```text
.claude-plugin/marketplace.json              # Claude Code marketplace
.agents/plugins/marketplace.json             # Codex marketplace
plugins/deliberate-engineering/
├── .claude-plugin/plugin.json                # Claude Code adapter
├── .codex-plugin/plugin.json                 # Codex adapter
├── commands/                                 # thin Claude Code entry points
├── codex/commands/                           # thin Codex command skills
└── skills/                                   # shared workflows, selectors and catalogs
```

Both manifests point at the same `skills/` directory. Claude Code loads the Markdown files in `commands/`; Codex loads corresponding skill entry points from `codex/commands/`, which makes them available through `/skills` and direct `$deliberate-engineering:<command>` mentions. Codex 0.154 does not create a direct `/deliberate-engineering:<command>` alias. That small duplication is intentional: Codex 0.154 silently skips migrated command templates containing Claude Code's `$ARGUMENTS`, while a native Codex skill receives the accompanying user request directly. Each pair is only a dispatch adapter and targets the same shared workflow. The `deliberate-engineering-host-context` skill is the other runtime adapter: it resolves the active host's personal data and transcript roots, then host-sensitive workflows use those resolved paths without probing the other host.
...
- **The router is the front door** (`/deliberate-engineering:start`). Before classifying anything it checks the altitude: a program too large for one session goes to orchestration, a concentrated cluster of irreversible steps goes to conduction, and only work that fits one session stays here. Then it classifies, names the phase sequence and the ceremony each phase earns, and routes (recommending, never forcing). By default, Rule 1 directs the agent to stop before an irreversible or outward-facing action that the workflow still recommends; an applicable explicit instruction or durable override that explicitly loosens Rule 1 can change that division of labor. At each phase transition it writes a short live note through the state skill below.
- **Five skills have no command, and are consulted rather than driven.** `deliberate-engineering-rules` is the constitution above, always in play and never invoked. `deliberate-engineering-voice` is the surface layer the communication selector reaches after the lenses have decided substance, and is described under *Communication and voice* below. `deliberate-engineering-overrides` reads your personal override file and applies it wherever a lens, a composition pattern or a standing rule is about to be used, declaring every deviation; what it cannot reach is the router's classification axes and its genre-to-phase map, which are architecture rather than content. `deliberate-engineering-state` owns the working-note that carries phase, sequence, ceremony band, chosen lenses and pendings across sessions; it writes that note to `.deliberate/state/` inside the repository once it confirms the path is VCS-ignored, which can mean adding that line to your `.gitignore`, and to `<host data root>/state/` when it cannot. `deliberate-engineering-host-context` resolves that host data root and the active session transcript without crossing host boundaries. They are named here because a map that only listed the commands would leave them invisible, and one of them writes to your repository.
- **Orchestration is its across-session sibling** (`/deliberate-engineering:orchestrate`). When the work is a program too large for one session, it runs an orchestration session that decomposes the work into units, dispatches each to a fresh worker session (a background subagent only for a narrow, earned band), verifies each return against primary evidence in a separate context, dispositions it, and tracks the whole program behind an always-current recovery anchor that lets any fresh session resume the orchestrator, committing that tracker to its own repository as each disposition lands. The router conducts phases *within* a session; this orchestrates units *across* sessions. It is deliberately thin: it cites the standing rules and reuses the selectors and the state working-note rather than restating them, and, under the default Rule 1 role split, it stops at the human gate for every outward or irreversible action a unit produces.
- **Conduction is its sibling for irreversibility clusters** (`/deliberate-engineering:conduct`). When irreversibility concentrates into a cluster (a merge cascade, a deploy chain, a batch of production data mutations, a teardown), it runs the cluster from a CONDUCTOR contract: a fixed step order with a gate between each, world state re-derived before every gate, the gate state kept in a per-item station table rather than in memory, each irreversible step bounded by a dry-run and a blast-radius limit before it fires and verified after, and, under the default Rule 1 role split, every recommended irreversible trigger handed to the operator. A `do not run` verdict records an alternative and queues no rejected action. It writes a conductor doc for the cluster, committed to the tracker's repository when one is in play. The router conducts phases *within* a session, orchestrate units *across* sessions, and this conducts steps *across an irreversibility cluster*, usually in one. It cites the verification rollout and data-mutation lenses and the planning ordering lenses rather than restating them; a small cascade stays in a tracker queue, and a cluster earns its own cockpit only when the gate graph outgrows one.

### What the plugin enforces, requires, and recommends

The plugin uses four kinds of force. Keeping them separate matters because a word such as “block” means different things in executable code and in instructions an agent is expected to follow.

| Category | Current example | Actual force |
|---|---|---|
| **Advice** | The router recommends phases, ceremony, and lenses with its rationale. | Judgment for the agent and operator; it has no independent blocking mechanism. |
| **Required process step** | For multi-phase or multi-session work, the state skill requires rehydration and checkpoints at named events. | A conforming agent performs the loaded prose contract. It is not a hook or a mechanical guarantee that the write occurs. |
| **Human gate** | By default, Rule 1 makes the agent prepare a consequential outward or irreversible action and stop for the operator when the workflow still recommends proceeding. | The method assigns who triggers the action. A `do not run` verdict queues no rejected action, and an applicable Rule 1 override can change the actor. The gate does not revoke credentials or tools. |
| **Deterministic repository check** | The consistency, invariant, host-compatibility, and version workflows return a failing status for the violations they parse. | Executable rejection under the inputs the check actually validates. Whether a CI failure prevents merge depends on repository policy requiring that status. Some historical comparisons deliberately pass after reporting that a usable base was unavailable. |

Workflow-local gates such as `promote` stopping on a suspected leak, and `conduct` returning `do not run`, are process contracts. Their `STOP`, `must`, and `block` language directs a conforming agent inside the named workflow; it does not install a sandbox, revoke a capability, or make arbitrary progress technically impossible.

An applicable explicit current-session instruction supersedes plugin defaults under the host's instruction hierarchy. A durable personal override is narrower: it reaches only named lenses, composition patterns, and standing rules, and can change the Rule 1 actor when its scope applies. Neither channel bypasses higher-priority host instructions, permissions, credentials, repository rules, or other external policy.

This distinction has an honest limitation: an agent can fail to load or follow a prose contract. The executable checks provide stronger evidence only for the properties they parse, when they run with the inputs those properties require.
...
flowchart TD
    ...
    voice{"is there a voice profile?<br/>&lt;host data root&gt;/voice/"}
...
- **Your overrides file is the personal layer.** Read at runtime, it takes precedence over any shipped lens, composition pattern or rule; when it changes what the agent does, the agent says so. The host-native `capture` entry point grows that file for you, reading the full retained session history from disk when resolvable and validated (your typed messages, extracted to a temporary directory it does not delete) and distilling it into ready-to-paste blocks. Its Codex adapter follows the thread store's active pointer and bounded `history_base` lineage, so a revert keeps retained history without resurrecting reverted turns; project-wide capture also resolves active or archived and plain or compressed rollouts, with `zstd` required only for compressed files.
- **The author tools grow the shared catalogs.** `/deliberate-engineering:contribute` generalizes a session's judgment into a candidate, dropping anything that cannot be said without the specifics. `/deliberate-engineering:promote` first runs a workflow-local blocking leak audit. On the isolated route it edits the catalog append-only, updates routing and the counts, metadata, and CHANGELOG the edit forces, then removes the candidate; on the structural route it edits none of those and recommends a full design cycle. The author tools' own contracts stop before commit, PR, or push. A durable override that changes Rule 1 does not by itself remove those workflow-local stops; an applicable current-session instruction remains subject to the host's instruction hierarchy.
...
- **Scale**: [Orchestrate](guides/orchestrate.md) runs a program too large for one session as dispatched units with one authoritative tracker; [Conduct](guides/conduct.md) runs a cluster of irreversible steps in a fixed, gated order where, under the default Rule 1 role split, you pull every recommended irreversible trigger.
...
One mental model runs across all of them, and it is the ruler rather than a shared set of axes: the cost of being wrong decides the depth, never line count (each phase classifies on its own axes, as above). The plugin recommends a depth and a set of lenses *with its reasoning*, and you stay in control. For multi-phase or multi-session work, it requires live-note checkpoints rather than trusting recall. Under the default Rule 1 role split, it also stops before a workflow-recommended irreversible or outward-facing action (a merge, a deploy, a push, a posted message); an applicable explicit instruction or durable Rule 1 override can change the actor. These are agent-executed process contracts, not runtime interlocks. The nine standing rules hold underneath every phase the whole time.

### Adapt: make it think like you

The plugin is opinionated, and it's meant to become yours. A personal file at `<host data root>/overrides.md` takes precedence over the shipped content it can address, which is the named lenses, the numbered composition patterns and the standing rules; the router's four routing axes and its genre to phase-sequence map are architecture and stay out of reach. Entries are addressed by stable identifiers: `review #N`, `verify #N`, `planning #N`, `debug #N`, `communication #N`, `<catalog> pattern #N` for a composition pattern, or `Rule N`. Three operations: `disable` turns a lens, composition pattern or rule off; `modify` appends your annotation alongside the shipped text, and where the two conflict your annotation governs; `add` defines your own. What a `disable` removes is a method, never a required field: disabling the lens that prescribes how to verify a rollback does not remove the conductor contract's requirement that a recovery path be verified, it removes the prescription for how. Two entries that contradict each other are named and asked about rather than silently resolved, which matters because the capture skill only ever appends, so collisions accumulate. And a direct instruction in the current session beats the file: the override file is durable preference, not an immutable mandate. The agent always declares when an override changed what it did (nothing happens silently), and you can even loosen a safety rule, which it honors while calling out the raised autonomy. A composition pattern is a different kind of content from a lens, and carries its own numbering: a lens is applied to the artifact under review, a pattern is applied to the lenses, prescribing how they chain. Each of the four phase catalogs ends in a numbered `Appendix: Composition Patterns` that states its own count; the communication catalog carries a prose composition note instead and therefore has no pattern targets, which is a deliberate exception rather than an omission. The numbers are the lens headings in the catalogs themselves, and for a pattern the numbered bullet in that catalog's composition appendix, which is where to look one up before writing an entry by hand: [review](../plugins/deliberate-engineering/skills/review-strategy-selector/catalog.md), [verification](../plugins/deliberate-engineering/skills/verification-strategy-selector/catalog.md), [planning](../plugins/deliberate-engineering/skills/planning-strategy-selector/catalog.md), [debug/operate](../plugins/deliberate-engineering/skills/debug-operate-strategy-selector/catalog.md), [communication](../plugins/deliberate-engineering/skills/communication-collaboration-selector/catalog.md).
...
One operation the block above does not show, and the two other shapes an `add` can take. `modify` keeps the shipped text intact and reads your annotation alongside it, in a field named `**Add:**`; where the two conflict, your annotation governs, because it is your practice. And an `add` header names the catalog, optionally followed by `pattern` when what you are adding composes lenses rather than examining an artifact:
...
- `/deliberate-engineering:promote` drives a candidate into the catalog: a blocking leak-audit first (any surviving specific stops it), then an append-only edit (a new lens gets the next free number and existing lenses are never renumbered, so the override identifiers you cite stay stable), the routing that makes the new lens reachable from its selector, a fresh-context subagent review, the count and manifest updates that edit forces, and the removal of the candidate it promoted. A structural change (a new catalog, a reorganization, a rule change) is not auto-applied; promote recommends the full design cycle instead.

Both tools' own workflow contracts stop before commit, PR, or push. A durable Rule 1 override does not by itself remove those stops; an applicable current-session instruction remains subject to the host's instruction hierarchy. Publication remains the operator's decision under the default Rule 1 role split. For the contributor workflow end to end, see [`CONTRIBUTING.md`](../CONTRIBUTING.md).
