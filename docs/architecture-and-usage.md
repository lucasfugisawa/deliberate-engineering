...
- **The author tools grow the shared catalogs.** `/deliberate-engineering:contribute` generalizes a session's judgment into a candidate (dropping anything that can't be said without the specifics) and `/deliberate-engineering:promote` runs a workflow-local, blocking leak audit, edits the catalog append-only, and follows that edit through everything it forces: the selector that routes the new lens, or the compose step that cites a new composition pattern, the counts stated in the README and in this document, all version-bearing plugin metadata, the CHANGELOG, and the deletion of the promoted candidate from the queue. Under the default Rule 1 role split, both stop at the human gate: they edit only the working tree and leave commit, PR, and push to the operator.
