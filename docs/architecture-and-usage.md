...
**About the state notes specifically.** One note per work unit, named after whatever identifies that unit: the branch, or a PR number or ticket key when the branch name is not a usable identifier, with slashes turned into hyphens, so work on `feat/user-export` leaves `feat-user-export.md`. The note is worth opening: it holds a live-state block that gets rewritten and an append-only decision log that records what was decided and why, which is exactly what a person resuming the work wants to read. Nothing prunes them, so they accumulate one per branch. And if you run a tracker or `superpowers`, the plugin delegates to it and writes less of its own.
