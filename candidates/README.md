...
Each file is one proposed lens, composition pattern or rule change (`status: pending`). The skill deposits files here but never commits them. Promotion to the catalog is gated: a human reviews the candidate, runs a leak-audit, and uses the `deliberate-engineering-promote` skill (which edits the catalog in the working tree and stops before commit/PR/push). Nothing here is part of the shipped catalog until it is promoted and merged.
